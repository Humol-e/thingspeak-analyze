import streamlit as st
import requests
import pandas as pd
import matplotlib.pyplot as plt
import plotly.graph_objects as go

st.set_page_config(layout='wide', initial_sidebar_state='expanded')
st.sidebar.header('Thingspeak analytycs `v1.0`')
st.sidebar.markdown('`demo`')


img = "data:image/jpeg;base64,"
st.sidebar.link_button("Documentation", "https://github.com/Humol-e/Graficator")

def get_data():
    r = f"https://api.thingspeak.com/channels/2338840/feeds.json?results=300000"
    response = requests.get(r)
    data = response.json()
    return pd.DataFrame(data['feeds'])

with st.sidebar:
    st.image(img)

get_data()
data = get_data()
df = pd.DataFrame(data)

st.title("Home")


def eliminar_filas_cero(df, Bedroom, Los_Mochis):
    return df[(df[Bedroom].notna()) & (df[Los_Mochis].notna())]

df = eliminar_filas_cero(df, "Bedroom", "Los_Mochis")
df = df.drop(df[df['Bedroom'] == 0].index)
df = df.drop(df[df['Los_Mochis'] == 0].index)
eliminar_filas_cero(df, Bedroom = "Bedroom", Los_Mochis = "Los_Mochis")

df["Bedroom"] = pd.to_numeric(df["Bedroom"], errors='coerce')
df = df[df['Bedroom'] != 0]

df["Los_Mochis"] = pd.to_numeric(df["Los_Mochis"], errors='coerce')
df = df[df['Los_Mochis'] != 0]

with st.expander("See dataframe"):
    st.dataframe(df, width=1000, height=500)
juanito = st.empty()


def graficar():
    figura = go.Figure()
    figura.add_trace(go.Scatter(x=df["ID"], y=df["Bedroom"], name="Bedroom"))
    figura.add_trace(go.Scatter(x=df["ID"], y=df["Los_Mochis"], name="Los_Mochis"))
    figura.update_layout(title="Graphic", xaxis_title="Entry ID", yaxis_title="Temperature")
    st.plotly_chart(figura)

graficar()